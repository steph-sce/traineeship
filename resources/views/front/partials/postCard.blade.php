    <div class="card my-2 bg-light" style="min-height: 15vh;">
        ...
        <div class="card-body">
            ...
            @if(count($post->picture) > 0)
                <img class="col-12 col-md-6 pt-3" src="{{asset('images/' . $post->picture->link) }}" alt="{{$post->picture->title}}">
                @else
                <div class="col-12 col-md-6">{{ __('No picture linked to this post') }}</div>
            @endif
            <div class="col-12 col-md-6 pt-3 float-right">
                <h4>{{$post->title}}</h4>
                <p>{{$post->description}}</p>

            </div>
            <ul class="list-group col-12 col-md-6 pt-3" style="padding-left: 15px">
